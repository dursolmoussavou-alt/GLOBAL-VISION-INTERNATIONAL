...
function openUserModal(){showModal(`<div class="modal-head"><h3>${db.online?'Enregistrer un collaborateur':'Ajouter un collaborateur local'}</h3><button class="btn btn-secondary" onclick="closeModal()">Fermer</button></div><form onsubmit="addUser(event)"><div class="form-grid"><div class="field"><label>Nom complet</label><input name="name" required></div><div class="field"><label>Email professionnel</label><input name="email" type="email" required></div><div class="field"><label>Mot de passe initial</label><input name="password" type="password" minlength="6" required></div></div><p class="note" style="margin-top:12px">Le rôle sera automatiquement défini sur <b>Collaborateur</b> et le compte sera activé.</p><button class="btn btn-primary" style="margin-top:18px">Créer le compte</button></form>`)}
async function addUser(e){e.preventDefault();const o=Object.fromEntries(new FormData(e.target).entries());if(db.online){const {data:{session}}=await sb.auth.getSession();if(!session)return toast('Session administrateur expirée.');try{const r=await fetch(`${FUNCTIONS_BASE}/create-collaborator`,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${session.access_token}`},body:JSON.stringify({full_name:o.name,email:o.email,password:o.password})});const out=await r.json().catch(()=>({}));if(!r.ok)throw new Error(out.error||'Création impossible.');closeModal();toast('Collaborateur créé avec succès.');await loadCollaborators();}catch(err){console.error(err);toast(err.message||'Impossible de créer le collaborateur.')}return}if(db.users.some(u=>u.email.toLowerCase()===o.email.toLowerCase()))return toast('Cet email existe déjà.');o.id='u-'+Date.now();o.role='collaborator';o.approved=true;db.users.push(o);saveLocal();closeModal();renderPage();toast('Utilisateur local créé.')}
async function loadCollaborators(){if(!db.online)return;const box=document.getElementById('collaboratorsTable');if(box)box.innerHTML='<div class="empty">Chargement…</div>';try{const {data,error}=await sb.from('profiles').select('id,full_name,email,role,approved,created_at').order('created_at',{ascending:true});if(error)throw error;const rows=(data||[]).map(u=>`<tr><td>${esc(u.full_name||'—')}</td><td>${esc(u.email||'—')}</td><td>${u.role==='admin'?'Administrateur':'Collaborateur'}</td><td>${u.approved?'Actif':'Bloqué'}</td></tr>`).join('');if(box)box.innerHTML=`<div class="table-wrap"><table class="table"><thead><tr><th>Nom</th><th>Email</th><th>Rôle</th><th>Statut</th></tr></thead><tbody>${rows||'<tr><td colspan="4">Aucun collaborateur.</td></tr>'}</tbody></table></div>`}catch(err){console.error(err);if(box)box.innerHTML='<div class="empty">Impossible de charger les collaborateurs.</div>';toast('Impossible de charger les collaborateurs.')}}
