...
const LOGO_FALLBACK = "assets/gvi-logo.jpg";
const FUNCTIONS_BASE = `${CFG.SUPABASE_URL || ""}/functions/v1`;
function logoTag(className="", alt="GVI International"){
  return `<img class="${className}" src="${LOGO_SRC}" alt="${esc(alt)}" onerror="this.onerror=null;this.src='${LOGO_FALLBACK}'">`;
}
function logoDataForPrint(){ return String(LOGO_SRC).startsWith("data:image/") ? LOGO_SRC : new URL(LOGO_SRC, window.location.href).href; }
...
function layout(){
 ...
 document.getElementById("app").innerHTML=`<div class="app-shell">
  <header class="topbar"><div class="topbar-left">${logoTag("logo-sm")}<div class="brand-text"><b>GVI INTERNATIONAL</b><span>Gestion & expédition de colis</span></div></div>
...
function loginView(msg=""){
 document.getElementById("app").innerHTML=`<div class="login"><div class="login-card"><div class="logo-frame">${logoTag("logo")}<strong>GVI</strong><span>INTERNATIONAL</span></div><h1>Gestion des colis</h1><p class="subtitle">Espace professionnel GVI International</p>
...
function go(p){if(p==="admin"&&!isAdmin()){toast("Accès administrateur requis.");return}state.page=p;state.selected=null;state.editing=null;renderPage();if(p==="admin"&&db.online)setTimeout(loadCollaborators,0);window.scrollTo({top:0,behavior:"smooth"})}
...
function labelHTML(p, logoSrc=LOGO_SRC){return `<div class="label-print"><img src="${logoSrc}" class="label-logo" alt="GVI International"><div class="label-brand">GVI INTERNATIONAL</div><div class="label-id">${esc(p.id)}</div><div class="label-row"><b>Nom :</b> ${esc(p.client)}</div><div class="label-row"><b>N° :</b> ${esc(p.tel||'—')}</div><div class="label-row"><b>Provenance :</b> ${esc(p.origin||'—')}</div><div class="label-row"><b>Destination :</b> ${esc(p.destination)}</div><div class="label-row"><b>Prix :</b> ${money(p.total)}</div><div class="label-row"><b>Poids :</b> ${esc(p.weight)} kg</div><div class="label-row"><b>Statut :</b> ${esc(p.status)}</div><div class="label-qr"><div id="printQR"></div></div></div>`}
function printLabel(id){const raw=db.parcels.find(x=>x.id===id);if(!raw)return;const p=normalizeParcel(raw),w=window.open('','_blank','width=500,height=700');if(!w){toast('Autorisez les fenêtres pop-up pour imprimer l’étiquette.');return}const html=labelHTML(p,logoDataForPrint());w.document.write(`<html><head><title>Étiquette ${esc(p.id)}</title><style>body{margin:0;font-family:Arial;background:#fff}.label-print{width:90mm;padding:8mm;margin:auto}.label-print img{width:50mm;height:auto;max-height:32mm;object-fit:contain;display:block;margin:0 auto 4mm}.label-brand{text-align:center;font-weight:800;margin:2mm}.label-id{font-size:22px;font-weight:900;text-align:center}.label-row{font-size:11px;margin:3px 0}.label-qr{display:flex;justify-content:center;margin:6mm 0}</style></head><body>${html}<script src="https://cdn.jsdelivr.net/npm/qrcodejs@1.0.0/qrcode.min.js"><\/script><script>new QRCode(document.getElementById('printQR'),{text:${JSON.stringify(parcelPayload(p))},width:220,height:220});setTimeout(()=>window.print(),700)<\/script></body></html>`);w.document.close()}

function admin(){if(!isAdmin())return '<div class="empty">Accès administrateur requis.</div>';return `<div class="page-head"><div><div class="eyebrow">ADMINISTRATION</div><h2>Administration</h2><p>Gérez les collaborateurs et les tarifs directement depuis GVI.</p></div></div><div class="two-col"><section class="card"><div class="section-head"><h3>Collaborateurs</h3><span class="mode-pill ${db.online?'online':'local'}">${db.online?'Comptes Supabase':'Comptes locaux'}</span></div><div class="actions" style="margin-bottom:15px"><button class="btn btn-primary" onclick="openUserModal()">＋ Enregistrer un collaborateur</button><button class="btn btn-secondary" onclick="loadCollaborators()">Actualiser</button></div><p class="note">Créez, activez, bloquez ou réinitialisez les comptes sans ouvrir Supabase. Les comptes créés ici sont automatiquement des <b>Collaborateurs</b>.</p>${db.online?`<div id="collaboratorsTable"><div class="empty">Chargement des collaborateurs…</div></div>`:`<div class="table-wrap"><table class="table"><thead><tr><th>Nom</th><th>Email</th><th>Rôle</th><th>Statut</th><th>Actions</th></tr></thead><tbody>${db.users.map(u=>`<tr><td>${esc(u.name)}</td><td>${esc(u.email)}</td><td>${u.role==='admin'?'Administrateur':'Collaborateur'}</td><td>${u.approved?'Actif':'Bloqué'}</td><td>${u.id!=='u-admin'?`<button class="btn btn-danger" onclick="deleteUser('${esc(u.id)}')">Supprimer</button>`:'Compte principal'}</td></tr>`).join('')}</tbody></table></div>`}</section>
...
function openUserModal(){showModal(`<div class="modal-head"><div><div class="eyebrow">NOUVEAU COMPTE</div><h3>Enregistrer un collaborateur</h3></div><button class="btn btn-secondary" type="button" onclick="closeModal()">Fermer</button></div><form onsubmit="addUser(event)"><div class="form-grid"><div class="field"><label>Nom complet</label><input name="name" autocomplete="name" required maxlength="120" placeholder="Ex. Jean Dupont"></div><div class="field"><label>Email professionnel</label><input name="email" type="email" autocomplete="email" required placeholder="collaborateur@gvi-international.com"></div><div class="field"><label>Mot de passe initial</label><input name="password" type="password" autocomplete="new-password" minlength="6" required placeholder="6 caractères minimum"></div></div><div class="account-info"><b>Rôle :</b> Collaborateur &nbsp;·&nbsp; <b>Statut :</b> Actif<br><span class="muted">Le collaborateur pourra se connecter immédiatement avec ces identifiants.</span></div><button id="createUserBtn" class="btn btn-primary" style="margin-top:18px">Créer le compte</button></form>`)}
async function callAdminFunction(action,payload={}){const {data:{session}}=await sb.auth.getSession();if(!session)throw new Error('Session administrateur expirée.');const r=await fetch(`${FUNCTIONS_BASE}/manage-collaborators`,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${session.access_token}`},body:JSON.stringify({action,...payload})});const out=await r.json().catch(()=>({}));if(!r.ok)throw new Error(out.error||'Opération impossible.');return out}
async function addUser(e){e.preventDefault();const o=Object.fromEntries(new FormData(e.target).entries());if(db.online){const btn=document.getElementById('createUserBtn');if(btn){btn.disabled=true;btn.textContent='Création…'}try{await callAdminFunction('create',{full_name:o.name,email:o.email,password:o.password});closeModal();toast('Collaborateur créé avec succès.');await loadCollaborators()}catch(err){console.error(err);toast(err.message||'Impossible de créer le collaborateur.');if(btn){btn.disabled=false;btn.textContent='Créer le compte'}}return}if(db.users.some(u=>u.email.toLowerCase()===o.email.toLowerCase()))return toast('Cet email existe déjà.');o.id='u-'+Date.now();o.role='collaborator';o.approved=true;db.users.push(o);saveLocal();closeModal();renderPage();toast('Utilisateur local créé.')}
async function loadCollaborators(){if(!db.online)return;const box=document.getElementById('collaboratorsTable');if(box)box.innerHTML='<div class="empty">Chargement…</div>';try{const out=await callAdminFunction('list');const rows=(out.users||[]).map(u=>`<tr><td><b>${esc(u.full_name||'—')}</b></td><td>${esc(u.email||'—')}</td><td>${u.role==='admin'?'Administrateur':'Collaborateur'}</td><td><span class="badge ${u.approved?'success':'danger'}">${u.approved?'Actif':'Bloqué'}</span></td><td class="row-actions">${u.role==='collaborator'?`<button class="btn btn-secondary" onclick="toggleUser('${esc(u.id)}',${u.approved})">${u.approved?'Bloquer':'Activer'}</button> <button class="btn btn-secondary" onclick="resetUserPassword('${esc(u.id)}','${esc(u.full_name||u.email)}')">Réinitialiser</button> <button class="btn btn-danger" onclick="deleteOnlineUser('${esc(u.id)}','${esc(u.full_name||u.email)}')">Supprimer</button>`:'Compte administrateur'}</td></tr>`).join('');if(box)box.innerHTML=`<div class="table-wrap"><table class="table"><thead><tr><th>Nom</th><th>Email</th><th>Rôle</th><th>Statut</th><th>Actions</th></tr></thead><tbody>${rows||'<tr><td colspan="5">Aucun compte.</td></tr>'}</tbody></table></div>`}catch(err){console.error(err);if(box)box.innerHTML='<div class="empty">Impossible de charger les collaborateurs.</div>';toast(err.message||'Impossible de charger les collaborateurs.')}}
async function toggleUser(id,approved){if(!confirm(`${approved?'Bloquer':'Activer'} ce collaborateur ?`))return;try{await callAdminFunction('set_status',{user_id:id,approved:!approved});toast(approved?'Collaborateur bloqué.':'Collaborateur activé.');await loadCollaborators()}catch(e){toast(e.message||'Impossible de modifier le statut.')}}
async function resetUserPassword(id,name){const password=prompt(`Nouveau mot de passe pour ${name} (6 caractères minimum) :`);if(password===null)return;if(password.length<6)return toast('Le mot de passe doit contenir au moins 6 caractères.');try{await callAdminFunction('reset_password',{user_id:id,password});toast('Mot de passe réinitialisé.')}catch(e){toast(e.message||'Impossible de réinitialiser le mot de passe.')}}
async function deleteOnlineUser(id,name){if(!confirm(`Supprimer définitivement le compte de ${name} ?\nCette action est irréversible.`))return;try{await callAdminFunction('delete',{user_id:id});toast('Collaborateur supprimé.');await loadCollaborators()}catch(e){toast(e.message||'Impossible de supprimer le collaborateur.')}}
